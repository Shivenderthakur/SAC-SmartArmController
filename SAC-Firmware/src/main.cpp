// SAC firmware: the ESP32 runs its own hotspot and receives what the app sends.
//
...
// Servos are not driven yet. Serial output is off unless built with SAC_LOG=1:
...
void handleLine(const char* via, IPAddress from, String line) {
  ...
  for (int i = 0; i < CHANNELS; i++) LOGF(" %s=%d", NAMES[i], angles[i]);
  LOGF("   raw=%s\n", line.c_str());
}
